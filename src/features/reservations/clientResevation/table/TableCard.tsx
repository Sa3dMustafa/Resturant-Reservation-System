...
export function TableCard({ table, selected, onSelect }: Props) {
  ...
  return (
    <button
      ...
      className={`
        group relative flex min-h-67.5
        flex-col items-center rounded-xl border
        bg-[#151515] p-3 text-start
        transition-all duration-200

        ${
          available
            ? "border-[#3f3f3f] hover:border-[#c99a2e] hover:bg-[#181818]"
            : "cursor-not-allowed border-[#333] opacity-45"
        }

        ${selected ? "border-[#c99a2e] bg-[#1c180e] ring-1 ring-[#c99a2e]" : ""}
      `}
    >
